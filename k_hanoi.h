...
class Towers
{
    ...
    bool is_everything_legal()
    {
        ...
        for (int i = 0; i < number_of_pegs; i++)
            for(int j = 0; j < towers[i].size();j++)
               // check every disk in one peg is legal or illegal
    ...
    void initialize(int m, int n)
    {
        ...
        for(int i = 0; i < m; i++ )
        {
            // creat the number of pegs, which we want
            deque<int>peg;
            towers.push_back(peg);
        }
        // creat the number of disks, which we want
        for (int i = 0; i < n; i++)
    ...
    void move_top_disk(int source, int dest)
    {
       ...
       // move the top disk to the destination
        ...
        // remove the top dish in the source peg
        towers[source].pop_front();
        // check every once we moved disk
    ...
    void Move(int number_of_disks, int source_peg_number, int destination_peg_number, deque <int> free_peg_numbers)
    {
        if (free_peg_numbers.size() != 0)
        {
            if (number_of_disks > 1)
            {
                ...
                // define the 'i' as the number of free peg(the end of free peg is that number)
                int i = free_peg_numbers.back();
                // the last free peg is the destination of m should go
                free_peg_numbers.pop_back();
                free_peg_numbers.push_back(destination_peg_number);
                Move(m, source_peg_number, i, free_peg_numbers);
                // the last free peg we have used, so we need to remove the last peg
                free_peg_numbers.pop_back();
                // move remain number of disk to the final destination
                ...
                // move the m number of dish to the final destination
